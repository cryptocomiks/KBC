...
| **Brief** | The 30-second read at the top of each investigation and of the PDF: a one-sentence **verdict**, key-figure tiles (owners, sanctions, PEP, leaks, high-risk countries, adverse media, crypto exposure), the **ultimate owners with their chain and effective %** (or, for a person, what they control), and the **red flags with the next step to take** (EDD, identity confirmation, UBO reconciliation, SAR…). |
...
| **KYC / AML-CFT questionnaire** | On each case, the client-risk questions of an LCB-FT file (nature and purpose of the relationship, activity sector, legal structure, beneficial owners, PEP, countries, channel, expected amounts, cash, source of funds, behaviour), pre-filled with what the screening already shows. Answers + automatic screening → **vigilance level** (simplified / standard / enhanced) with the triggers, the **measures required** and the **next review date** (3 / 2 / 1 year). Printed in the PDF and shown on the dashboard. |
| **Navigation** | Every screen has its own address (`#/search?q=…`, `#/investigate?ids=…`, `#/cases/<id>/investigation`…): reload, back / forward buttons and shared links work. **Ctrl K / ⌘K** opens a quick search from anywhere (open a case, search a company, a person or an identifier). A case opens on a status header (validation step, vigilance, risk, documents received) with three tabs: *KYC file* (validation, questionnaire, checklist), *Investigation* and *History & notes* (changes, notes, validation steps, screening decisions). |
| **Validation workflow (four eyes)** | Each case is *to complete* → *awaiting validation* → *validated*, or *sent back* with a comment. It can be sent only once the questionnaire is answered and the required documents are received, and it must be validated by **someone other than the sender**. Every step is signed and dated (audit trail, printed in the PDF). A validated case whose monitoring finds a new sanctions / PEP / leak hit is **reopened automatically**. The cases page opens on work queues: *sensitive* (PEP / sanctions), *to validate*, *to complete*, *review due* (within 30 days) and *validated*. |
| **Case checklist** | The documents the findings call for (register extract, articles, UBO declaration, ID of each representative and beneficial owner…), ticked when received, and **additional diligences** suggested by the vigilance level and the red flags (transaction samples, source of funds, cash, payment methods, group chart…) — optional, removable, with your own additions. Each tick is signed and dated, and printed in the PDF. |
| **Bulk import** | Upload a client list (**CSV or Excel**, up to 500 lines; columns name / raison sociale, SIREN / SIRET / LEI / UID / company number, country, reference): one case per client. Each line is found in the registers (identifier = exact match; a name is matched automatically only when there is no doubt, otherwise the analyst picks the company) and then screened, **one step per call** so that every call fits in the host's time limit — driven by the import screen while it is open and finished by the daily monitoring job. The file is read in memory and not kept. |
...
| **Client documents vs registries** | Upload a register extract (Kbis…), a UBO declaration or a PSC register (text PDF): officers and owners are read by rules (no AI, nothing stored), corrected in an editable table if needed, then compared with the registries — consistent, different %, not in registries, **not declared** (undeclared beneficial owner). |
| **Report templates** | Full due diligence, KYC/KYB (standard), **enhanced due diligence** (source of wealth / source of funds, senior management approval), **periodic review** (changes since last review) — each with the documents to request and a sign-off block (four-eyes). |
| **Suspicious activity report draft** | Pre-filled draft for **TRACFIN** (France), **MROS** (Switzerland) or the **Luxembourg CRF**: subject, related persons, grounds for suspicion, chronology, flows, fields for bank transactions and measures taken, tipping-off warning. Never filed automatically. |
